...
let nextId = 1, startedAt = Date.now(), roundEnded = false;
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => { let data; try { data = JSON.parse(raw); } catch { return; }
    ...
    if (!player || player.dead || roundEnded) return;
    ...
    if (data.type === 'shoot') { const victim = players.get(data.target); if (!victim || victim.dead || victim.id === player.id) return; victim.health -= 34; broadcast({ type: 'hit', victim: victim.id }); if (victim.health <= 0) { victim.dead = true; player.kills++; broadcast({ type: 'kill', killer: player.id, killerName: player.name, victim: victim.id, victimName: victim.name }); setTimeout(() => { if (players.has(victim.id) && !roundEnded) { victim.dead = false; victim.health = 100; spawn(victim); broadcast({ type: 'respawn', victim: victim.id, x: victim.x, y: victim.y, z: victim.z }); } }, 3000); } }
  });
  ws.on('close', () => { if (player) { players.delete(id); broadcast({ type: 'feed', text: `${player.name} 离开了战区` }); } });
});
setInterval(() => { if (!roundEnded && Date.now() - startedAt >= 600000) { roundEnded = true; const winner = [...players.values()].sort((a, b) => b.kills - a.kills)[0]; broadcast({ type: 'roundEnd', winner: winner ? winner.name : '无人', kills: winner ? winner.kills : 0 }); } broadcast(snapshot()); }, 50);
