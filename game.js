...
const walls = []; function box(x, z, w, d, h = 3, color = 0x203d4e) { const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), new THREE.MeshStandardMaterial({ color, metalness: .55, roughness: .4 })); m.position.set(x, h / 2, z); m.castShadow = m.receiveShadow = true; scene.add(m); walls.push({ x, z, w, d }); }
box(0, -24, 50, 1, 5); box(0, 24, 50, 1, 5); box(-24, 0, 1, 50, 5); box(24, 0, 1, 50, 5); box(0, 0, 3, 12, 3.5, 0x294c5b); box(-11, -7, 7, 3); box(11, 7, 7, 3); box(-12, 10, 3, 8); box(12, -10, 3, 8); box(-5, 16, 8, 2); box(5, -16, 8, 2);
...
function collides(x, z) { return walls.some(b => x > b.x - b.w / 2 - .45 && x < b.x + b.w / 2 + .45 && z > b.z - b.d / 2 - .45 && z < b.z + b.d / 2 + .45); }
...
function receive(data) { if (data.type === 'welcome') { myId = data.id; joined = true; menu.classList.add('hidden'); hud.classList.remove('hidden'); camera.position.set(data.x, 1.6, data.z); document.body.requestPointerLock(); notice('死斗模式：所有玩家均为敌人'); }
 ...
 if (data.type === 'hit' && data.victim === myId) { document.querySelector('#health').textContent = Math.max(0, +document.querySelector('#health').textContent - 34); document.querySelector('#health').classList.toggle('low', +document.querySelector('#health').textContent <= 34); }
...
function die() { alive = false; respawn.classList.remove('hidden'); let n = 3; const el = document.querySelector('#respawn-count'); const timer = setInterval(() => { el.textContent = --n; if (!n) { clearInterval(timer); alive = true; document.querySelector('#health').textContent = 100; document.querySelector('#health').classList.remove('low'); respawn.classList.add('hidden'); } }, 1000); }
...
function addImpact(point) {
  const spark = new THREE.Mesh(new THREE.SphereGeometry(.09, 6, 6), new THREE.MeshBasicMaterial({ color: 0xefffff, transparent: true })); spark.position.copy(point); scene.add(spark); effects.push({ mesh: spark, life: .12, expand: true });
}
...
addEventListener('keydown', e => { keys[e.code] = true; if (['Space', 'KeyW', 'KeyA', 'KeyS', 'KeyD'].includes(e.code)) e.preventDefault(); if (e.code === 'KeyR') reload(); if (e.code === 'Space' && alive && camera.position.y <= standingHeight + .01) verticalVelocity = jumpSpeed; }); addEventListener('keyup', e => keys[e.code] = false);
...
function loop(time) { requestAnimationFrame(loop); remotes.forEach(remote => animateRemote(remote, time)); effects.forEach(effect => { effect.life -= .016; if (effect.expand) effect.mesh.scale.multiplyScalar(1.16); if (effect.mesh.material) effect.mesh.material.opacity = Math.max(0, effect.life * 10); }); for (let i = effects.length - 1; i >= 0; i--) if (effects[i].life <= 0) { scene.remove(effects[i].mesh); effects.splice(i, 1); } gunKick = Math.max(0, gunKick - .12); gun.position.z = -.72 + gunKick * .13; gun.rotation.x = -.12 - gunKick * .13; if (joined && alive) { const forward = new THREE.Vector3(); camera.getWorldDirection(forward); forward.y = 0; forward.normalize(); const right = new THREE.Vector3().crossVectors(forward, camera.up).normalize(); velocity.set(0,0,0); if (keys.KeyW) velocity.add(forward); if (keys.KeyS) velocity.sub(forward); if (keys.KeyD) velocity.add(right); if (keys.KeyA) velocity.sub(right); const crouching = keys.KeyC && camera.position.y <= standingHeight + .01; const targetHeight = crouching ? crouchingHeight : standingHeight; const speed = (keys.ShiftLeft && !crouching ? .19 : .105) * (crouching ? .55 : 1); verticalVelocity -= gravity * .016; camera.position.y += verticalVelocity * .016; if (camera.position.y <= targetHeight) { camera.position.y = targetHeight; verticalVelocity = 0; } if (velocity.lengthSq()) { velocity.normalize().multiplyScalar(speed); const nx = camera.position.x + velocity.x, nz = camera.position.z + velocity.z; if (!collides(nx, camera.position.z)) camera.position.x = nx; if (!collides(camera.position.x, nz)) camera.position.z = nz; } if (socket.readyState === WebSocket.OPEN && time - lastNetwork > 45) { socket.send(JSON.stringify({ type: 'move', x: camera.position.x, y: camera.position.y, z: camera.position.z, yaw: camera.rotation.y, pitch: camera.rotation.x, crouching })); lastNetwork = time; } } renderer.render(scene, camera); }
